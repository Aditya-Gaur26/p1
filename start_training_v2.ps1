...
if (-Not (Test-Path $v1ModelPath)) {
    Write-Host "  ERROR: V1 model not found at: $v1ModelPath" -ForegroundColor Red
...
Write-Host "  V1 model found" -ForegroundColor Green
Write-Host "  Setting V1 model to read-only (protection)" -ForegroundColor Green
Get-ChildItem "models\fine_tuned" -Recurse -File | ForEach-Object {
    $_.IsReadOnly = $true
}

Write-Host "  V1 model location: models\fine_tuned (PROTECTED)" -ForegroundColor Cyan
Write-Host "  V2 output location: $v2OutputPath (NEW)" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  Checkpoints save to: $v2OutputPath\checkpoint-*" -ForegroundColor Green
Write-Host "  Safe format: safetensors only (no .pt/.pth)" -ForegroundColor Green
Write-Host "  Auto-resume: Enabled from latest V2 checkpoint" -ForegroundColor Green
Write-Host "  V1 model: Protected from modifications" -ForegroundColor Green
Write-Host ""
...
Write-Host "  V1 model permissions restored" -ForegroundColor Green

# Verify V1 model wasn't modified
Write-Host "Verifying V1 model integrity..." -ForegroundColor Yellow
if (Test-Path $v1ModelPath) {
    Write-Host "  V1 model intact at: models\fine_tuned" -ForegroundColor Green
} else {
    Write-Host "  WARNING: V1 model missing (unexpected)" -ForegroundColor Red
}
...
if (Test-Path "$v2OutputPath\adapter_model.safetensors") {
    Write-Host "  V2 model saved to: $v2OutputPath" -ForegroundColor Green
    
    # Count checkpoints
    $checkpoints = Get-ChildItem "$v2OutputPath\checkpoint-*" -Directory -ErrorAction SilentlyContinue
    if ($checkpoints) {
        Write-Host "  Checkpoints created: $($checkpoints.Count)" -ForegroundColor Green
    }
} else {
    Write-Host "  WARNING: V2 model not found (training may have failed)" -ForegroundColor Red
}
